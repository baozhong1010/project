...
class Tudi_Zhuan_Rang(object):
    ...
    def parse(self,html,headers):
        ...
        for content in contents.find_all('tr')[1:]:
            ...
            old_tudi_user = content.find_all('td')[3].text.strip()
            if '..' in old_tudi_user:
                old_tudi_user = content.find_all('td')[3].span['title']
            new_tudi_user = content.find_all('td')[4].text.strip()
            if  '..' in new_tudi_user:
                new_tudi_user = content.find_all('td')[4].span['title']
            print xing_zheng_qu,tudi_location,old_tudi_user,new_tudi_user
            # print detail_text
            sql = "replace into Tudi_Zhuan_Rang values (%s,%s,%s,%s,%s,now())"
            self.save_to_sql(sql,(xing_zheng_qu,tudi_location,old_tudi_user,new_tudi_user,
                                  detail_text))
