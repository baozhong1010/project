...
class Tudi_Diya(object):
    ...
    def parse(self,html,headers):
        ...
        for content in contents.find_all('tr')[1:]:
            ...
            diya_mianji = content.find_all('td')[3].text.strip()
            if '..' in diya_mianji:
                diya_mianji = content.find_all('td')[3].span['title']
            diya_tudi_use = content.find_all('td')[4].text.strip()
            print xing_zheng_qu,tudi_location,diya_mianji,diya_tudi_use
            # print detail_text
            sql = "replace into Tudi_Diya values (%s,%s,%s,%s,%s,now())"
            self.save_to_sql(sql,(xing_zheng_qu,tudi_location,diya_mianji,diya_tudi_use,
                                  detail_text))
