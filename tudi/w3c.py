...
class CSS2Properties(dict, PyV8.JSClass):

    def __init__(self, d={}, dft=None):
        for k, v in d.items():
            if isinstance(v, dict):
                self.__setitem__(k, CSS2Properties(v, dft))
            else:
...
class Element(Node):
    ...
    @property
    def href(self):
        return urljoin(self.doc.window.url, self.tag['href'])
    ...
    @property
    def parentNode(self):
        # print type(self), type(self.tag), type(self.tag.parent)
        return DOMImplementation.createHTMLElement(self, self.tag.parent) if self.tag.parent else None
        # return self.tag.parent

    @property
    def parentElement(self):
        # print self.tag.name, self.tag.parent.name
        # if self.tag.parent.name == '[document]':
        #     return None
        return DOMImplementation.createHTMLElement(self, self.tag.parent) if self.tag.parent else None
        # return self.tag.parent
...
class HTMLDocument(Document):
    ...
    def __init__(self, doc, win=None, referer=None, lastModified=None, cookie=None):
    ...
    @property
    def cookie(self):
        # print '2[' + self._cookie + ']'
        return '; '.join([k.name + '=' + k.value for k in self._cookie])

    @cookie.setter
    def cookie(self, cookie):

        # print 'SetCookie >', cookie
        # print '_cookie >', self._cookie
        # print 'url', urlparse(self.URL).hostname
        cookie_name, cookie_val = re.split('[=;]', cookie)[:2]
        self._cookie.set(cookie_name, cookie_val, domain=urlparse(self.URL).hostname)
...
#
    ...
    def testElement(self):
        ...
        print '-' * 100
        # print self.doc.getElementsByName('div')
        # self.assert_(div)
        # self.failIf(div.hasChildNodes())
        # self.assertEquals(0, len(div.childNodes))
        # a = self.doc.createTextNode("12345")
        # div.appendChild(a)
