...
# def replace_add_event_listener(text):
#     print '*****', text
#     # print re.compile(u"document\.addEventListener\([^,]+,[^,]+,[^,]+\)").search(text)
#     # return ''
#     res = re.compile(u"(window|document)\.addEventListener\([^,]+,[^,]+,[^,]+\)").sub(lambda match:
#                                                                                      match.group(0).split(',')[1].strip()+'()', text)
#     print 'res', res
#     return re.compile(u"(window|document)\.addEventListener\([^,]+,[^,]+,[^,]+\)").sub(lambda match:
#                                                                                      match.group(0).split(',')[1].strip()+'()', text)
#
# PyV8.JSExtension("replace_add_event_listener/python", "native function replace_add_event_listener(s);", lambda secondfunc: replace_add_event_listener, register=True)
...
class Navigator(PyV8.JSClass):
    ...
    def fetch(self, url):
        # print url
        r = self.session.get(url, headers={'User-Agent': self.userAgent, 'Referer': self._win.url})
        return r
        ...
        #
        # if hasattr(self._win, "doc") and self._win.doc.cookie:
...
class Firefox(Navigator):
    ...
    @property
    def userAgent(self):
        return "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.11; rv:56.0) Gecko/20100101 Firefox/56.0"
...
class HtmlWindow(PyV8.JSClass):
    ...
    def __init__(self, url, dom_or_doc=None, navigator_or_class=Firefox, name="", target='_blank',
        ...
        if dom_or_doc is None:
            r = self._navigator.fetch(url)
            dom_or_doc = BeautifulSoup.BeautifulSoup(r.text)
        self.doc = w3c.getDOMImplementation(dom_or_doc, cookie=self._navigator.session.cookies, **kwds) if isinstance(dom_or_doc, BeautifulSoup.BeautifulSoup) else dom_or_doc
    ...
    def context(self):
        if not hasattr(self, "_context"):
            # self._context = PyV8.JSContext(self, extensions=['replace_add_event_listener/python'])
            self._context = PyV8.JSContext(self)
            self._context.enter()
            self._context.eval(open(os.path.join(os.path.dirname(__file__), 'my_env.js')).read())
            # with self._context as ctxt:
            #     ctxt.eval(open(os.path.join(os.path.dirname(__file__), 'my_env.js')).read())
...
class HtmlWindowTest(unittest.TestCase):
    ...
    def testScript(self):
        ...
        tag = self.doc.getElementById('id1')
        #
        self.assertEquals(u'DIV', tag.nodeName)
        # print self.win.context
        # print self.win.evalScript(u"(function() {eval('x=100');return x;}())")

    # def testTimer(self)z
    #     pass
